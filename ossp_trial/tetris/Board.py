...
class Board:
    ...
    def absorb_piece(self):
        ...
        self.level = (self.score//100)+1 #점수 100점 마다 level up
        if self.score >= 3:
            self.plane()
        if self.skill < 100:
            self.skill += 5
    ...
    def newGame(self,input_id):


        fontObj = pygame.font.Font('assets/Roboto-Bold.ttf', 30)
        textSurfaceObj = fontObj.render('Tetris Start', True, GREEN)
        textRectObj = textSurfaceObj.get_rect()
        textRectObj.center = (175, 185)

        fontObj1 = pygame.font.Font('assets/Roboto-Bold.ttf', 16)
        textObj1 = fontObj1.render("Player : " +input_id, True, GREEN)
        textRectObj1 = textObj1.get_rect()
        textRectObj1.center = (175, 230)

        fontObj2 = pygame.font.Font('assets/Roboto-Bold.ttf', 16)
        textSurfaceObj2 = fontObj2.render('Press a key to continue', True, GREEN)
        textRectObj2 = textSurfaceObj2.get_rect()
        textRectObj2.center = (175, 250)

        self.screen.fill(BLACK)
        self.screen.blit(textSurfaceObj, textRectObj)
        self.screen.blit(textSurfaceObj2, textRectObj2)
        self.screen.blit(textObj1, textRectObj1)
    ...
    # def plane(self):
    #     #plane_su=0 #plane_su = self.level/3 >=1:
    #     #t = threading.Thread(target=self.plane)
    #     #t.start()
    #
    #     #비행기 이미지와 처음 좌표 설정
    #     plane = pygame.transform.scale(pygame.image.load("assets/images/plane.png"), (25, 25))
    #     plane_position = (self.planex, self.planey)
    #     self.screen.blit(plane, plane_position)
    #
    #     #소리설정
    #     plane_sound = pygame.mixer.Sound('assets/sounds/bomb.wav')
    #     plane_sound.play()
    #
    #     #화면에 띄움
    #     self.board = []
    ...
    #     while True:
    #       if datetime.timedelta(seconds=1) <= datetime.now()-last_moved_time:
    #       # plane_position[0] += 1
    #         last_moved_time = datetime.now()
    #         plane_position=(self.planex+(datetime.now()-last_moved_time),self.planey)
    #         self.screen.blit(plane, plane_position)
    #         pygame.display.update()
    #         if self.planex == 20:
    #           break
        ...
        #비행기와 부딪힘 점검
        #   self.collide_with_plane(self, self.planex, self.planey)

    #
    # def collide_with_plane(self, x, y):
    #         if x < self.planex :
    #             return Board.COLLIDE_ERROR['left_wall']
    #         elif x >= self.planex :
    #             return Board.COLLIDE_ERROR['right_wall']
